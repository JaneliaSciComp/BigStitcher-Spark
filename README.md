...
<code>./estimate-flatfield -x ~/SparkTest/dataset.xml --maxViews 200 -o ~/SparkTest/flatfields.zarr -s ZARR</code>

Run `estimate-flatfield` without parameters for the full list of arguments. `--maxViews` randomly sub-samples that many planes per group (with `--seed` for reproducibility) to keep the estimate fast; `--workingSize` downsamples each plane to a square working resolution for the solve (`0` = full resolution). The BaSiC parameters `--lambda`/`--lambdaDarkfield` (`0` = auto), `--maxIterations`, `--optTol`, `--reweightTol`, `--maxReweightIterations` and `--epsilon` control the optimization, and `--estimateDarkfield` toggles darkfield estimation. Output format follows the usual conventions: `-s` selects `ZARR` (v3), `ZARR2` (v2), `N5` or `HDF5` (guessed from the `-o` extension), `-c`/`-cl` set compression, and `--blockSize`/`--blockScale`/`--useSharding` control chunking and Zarr v3 sharding. You can restrict which views are used with `--channelId`, `--illuminationId`, `--tileId`, `--angleId`, `--timepointId` or `-vi`.
