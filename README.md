...
* [**Usage**](#usage)
  ...
  * [Flatfield Correction](#flatfield)
    * [Estimate Flatfield/Darkfield](#estimate-flatfield)
    * [Apply Correction](#correct-flatfield)
...
## Flatfield Correction<a name="flatfield">

Light-sheet/SPIM tiles usually carry a spatially varying illumination profile: a multiplicative *flatfield* and an additive *darkfield* (background/dark offset). Correcting these before stitching and fusion removes seams and intensity gradients between tiles. BigStitcher-Spark implements the [BaSiC](https://www.nature.com/articles/ncomms14836) algorithm in two steps: first **estimate** one flatfield/darkfield per illumination profile, then **apply** the correction to every view and write a corrected copy of the dataset.

Both tools group views by their `(channel, illumination)` combination, so a multi-channel acquisition gets an independent flatfield/darkfield per channel automatically — you do not need to run them once per channel.

### Estimate Flatfield/Darkfield<a name="estimate-flatfield">

`estimate-flatfield` reads a selection of views, collects their 2D image planes (each z-slice is one sample), and solves the BaSiC low-rank + sparse decomposition on the driver (this step is *not* Spark-distributed — all planes feed a single optimization). It writes one `flatfield` and one `darkfield` dataset per group into an N5/Zarr/HDF5 container, laid out as `channel<C>/illumination<I>/flatfield` and `.../darkfield`. A typical call:

<code>./estimate-flatfield -x ~/SparkTest/dataset.xml --numViewSetups 10 --numFrames 200 -o ~/SparkTest/flatfields.zarr -s ZARR</code>

Run `estimate-flatfield` without parameters for the full list of arguments. The input stack for each group is chosen in two random steps (both reproducible via `--seed`): `--numViewSetups` randomly picks that many view setups from the group, and `--numFrames` then randomly picks that many z-slices total from the chosen setups (both default to "all"). To estimate from *specific* view setups instead of random ones, pin them with the usual `--channelId`, `--illuminationId`, `--tileId`, `--angleId`, `--timepointId` or `-vi` filters (omit `--numViewSetups`) and optionally still random-sample frames with `--numFrames`. `--workingSize` downsamples each plane to a square working resolution for the solve (`0` = full resolution). The BaSiC parameters `--lambda`/`--lambdaDarkfield` (`0` = auto), `--maxIterations`, `--optTol`, `--reweightTol`, `--maxReweightIterations` and `--epsilon` control the optimization, and `--estimateDarkfield` toggles darkfield estimation. Output format follows the usual conventions: `-s` selects `ZARR` (v3), `ZARR2` (v2), `N5` or `HDF5` (guessed from the `-o` extension), `-c`/`-cl` set compression, `--blockSize` controls chunking, and `--useSharding`/`--blockScale` can explicitly enable and size Zarr v3 shards.

### Apply Correction<a name="correct-flatfield">

`correct-flatfield` reads the estimated fields and applies `corrected = max((raw - darkfield) / flatfield, 0)` to every (selected) view via Spark, writing a corrected multi-resolution copy and an updated BigStitcher XML that points at it. The correction is computed at full resolution (s0) and the resolution pyramid is generated by downsampling the *corrected* s0 — this is more accurate than correcting each level independently. A typical call:

<code>./correct-flatfield -x ~/SparkTest/dataset.xml --fields ~/SparkTest/flatfields.zarr -o ~/SparkTest/corrected.ome.zarr --localSparkBindAddress</code>

Run `correct-flatfield` without parameters for all arguments. `--fields` is the container written by `estimate-flatfield`. The corrected dataset reuses the same ViewSetup IDs, so it is a drop-in replacement and existing registrations/transforms still apply; by default the new XML is written next to the input as `<input>.corrected.xml` (override with `-xo`). **By default the output format is inherited from the source volume** — storage type, chunk size, sharding on/off and shard size, compression, data type, and the downsampling pyramid are read from the input and reproduced. Any of `-s`, `--blockSize`, `--blockScale`, `--useSharding`, `-c`/`-cl`, `-ds` and `--outputDataType` (`SAME` or `FLOAT32`) override the inherited values when set. `--missingFields` chooses what happens for a group that has no estimated field: `ERROR` (default), or `SKIP` (leave those views out of both processing and the emitted XML). As with the other tools you can restrict views with `--channelId`, `--illuminationId`, `--tileId`, `--angleId`, `--timepointId` or `-vi`, and `--dryRun` tests without writing.

**Temporal baseline drift** (BaSiC's per-image background / "basefluor" step) can additionally be removed at correction time with `--baselineDrift`. This is the equivalent of the Fiji BaSiC plugin's "temporal drift of baseline" option and is applied *after* the flatfield/darkfield correction: `corrected = max((raw - darkfield)/flatfield - baseline, 0)`. The per-image background is estimated from the data being corrected (not from the estimation sample), as a robust percentile of the shading-corrected pixels (default: median, `--baselinePercentile`), which correctly tracks the background for both fluorescence (dark background with sparse bright signal) and brightfield-like data (bright-majority background). Modes:
- `IGNORE` (default) — no baseline removal (identical to omitting the option).
- `ZERO` — estimate each image's background and subtract it completely.
- `MEAN` — subtract each image's background but restore the mean background across the group (removes drift/flicker while keeping the overall level; matches BaSiC's "replace with temporal mean", recommended for brightfield).

`--baselineGranularity` chooses `SLICE` (default; one background per z-plane, so depth-dependent background is removed) or `VIEW` (one background per view/tile). `--baselineScale` sets the coarse XY size used for the estimate (default 128; z always full). The "mean" in `MEAN` is taken per `(channel,illumination)` group over all corrected images. Since fields are typically estimated and applied per acquisition round (via `--timepointId`), the drift being leveled is the tile-to-tile (and slice-to-slice) background variation within each round. Per-view baselines and the per-group reference mean are written to the output container as attributes for QC.

<code>./correct-flatfield -x ~/SparkTest/dataset.xml --fields ~/SparkTest/flatfields.zarr -o ~/SparkTest/corrected.ome.zarr --baselineDrift MEAN --localSparkBindAddress</code>
